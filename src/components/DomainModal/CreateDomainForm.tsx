...
const CreateDomainForm: React.FC = () => {
  ...
  const {
    data,
    refetch: refetchDomainCategories,
    isFetching: isFetchingDomainCategories,
  ...
  const domainSubCategories = Array.isArray(
    subCategoryData?.data.domainSubCategories,
  )
    ? subCategoryData.data.domainSubCategories
    : [];
  ...
  const filteredSubCategories = selectedCategoryId
    ? domainSubCategories.filter(
        (subCategory) => subCategory.domain_category._id === selectedCategoryId,
      )
    : [];
  ...
  return (
    <>
      ...
      <Form form={domainForm} layout="vertical" onFinish={handleCategorySubmit}>
        <Form.Item
          name="name"
          label="Domain Name"
        ...
        <Form.Item>
          <div className="flex justify-end gap-2">
            <Button onClick={handleCancel} disabled={isDomainMutationLoading}>
              Cancel
            </Button>
            <Button
              type="primary"
              htmlType="submit"
              loading={isDomainMutationLoading}
            >
              Create Domain
            </Button>
          </div>
